...
df_pivot = df.pivot_table(index="node_id", columns=pd.Grouper(key = "recv_time", freq = "20min"), values="temp_c", aggfunc="mean")
df_pivot = df_pivot.reindex(node_names) # ノードの並び順を固定

df_pivot = df_pivot.ffill(axis=1, limit=2).bfill(axis=1, limit=2)
