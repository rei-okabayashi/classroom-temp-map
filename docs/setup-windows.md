...
> **学校PCにも、同じ手順で導入します。** 自宅PC分はこの手順書どおり事前宿題として各自で、学校PC分はDay 0（8/17）の残り時間と8/18（火）の放課後に**チームで一緒に**導入します（インストーラは企画者がUSBメモリで持参します）。学校PCは1人1アカウントなので、GitHubの認証も自宅と同じ手順で大丈夫です。管理者権限の画面が出て進めなくなったときは、企画者に声をかけてください（8/17より前に企画者が学校PC1台で導入できるか下見をしておきます。もしドライバが学校PCに入れられない場合は、ボードへの書き込みだけ自宅PCや企画者PCで行う運用に切り替えます）。
...
4. ダウンロードした `.exe` をダブルクリックしてインストールします。表示される画面はすべて既定値のままで進めてOKです（ライセンス同意 → インストール場所 → インストール実行）。
5. インストール後、Arduino IDE を一度起動して、画面が開くことを確認します。
...
3. VSCode を開き、メニュー `ターミナル`（Terminal）→ `新しいターミナル`（New Terminal）でターミナルを開きます。
4. リポジトリを置きたい場所（例：`ドキュメント` フォルダ）に移動してから、以下のように clone します。
...
5. clone が終わったら、VSCode の `ファイル`（File）→ `フォルダーを開く`（Open Folder）で、clone してできた `classroom-temp-map` フォルダを開きます。

### 名前とメールを設定する

Gitに「誰の変更か」を教えるための設定です。これをしておかないと、初めてcommitしようとしたときに `Please tell me who you are` というエラーで止まってしまいます。

1. VSCode のターミナル（上の手順3で開いたもの）に、以下を1行ずつ入力してEnterを押します（引用符の中は自分のユーザー名・0-2でメモしたアドレスに書き換えます）。
...
2. 以下を実行します。
   ...
   出力に `user.name=...` と `user.email=...` の2行が含まれていればOKです。含まれていない場合は、1の2行をもう一度実行してみてください。

### リポジトリへの招待を受け取る（8/16頃）

このリポジトリは企画者が作成し、あとからメンバーを招待する形になっています。

1. 企画者があなたのGitHubユーザー名（0-4で報告したもの）を使って招待すると、登録したメールアドレス宛に「invited you to collaborate」という件名のメールが届きます。
2. メール本文の「Accept invitation」ボタンを押します。
3. メールが見当たらない場合は、[https://github.com/notifications](https://github.com/notifications) を開いて確認します。
...
7. 以下のような出力が表示されれば、書き込みと動作確認は完了です。
   ...
   （読み下し：このノードが起動したことを知らせる見出し行は「onmura node」。ID は n1、データの形式は v1版。最後の行は、このボード固有のMACアドレス＝機器固有の番号です。MACアドレスは次の段落でくわしく説明します。）
