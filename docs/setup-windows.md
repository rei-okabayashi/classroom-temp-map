...
> **学校PCにも、同じ手順で導入します。** 自宅PC分はこの手順書どおり事前宿題として各自で、学校PC分はDay 0（8/17）の残り時間と8/18（火）の放課後に**チームで一緒に**導入します（私有USBメモリ・個人PCは学校ルールで持ち込み禁止のため、インストーラは各自が学校回線で公式サイトからダウンロードします。回線が混まないよう、開始のタイミングをずらします）。学校PCは1人1アカウントなので、GitHubの認証も自宅と同じ手順で大丈夫です。管理者権限の画面が出て進めなくなったときは、企画者に声をかけてください（8/17より前に企画者が学校PC1台で導入できるか下見をしておきます。もしドライバが学校PCに入れられない場合は、**ボードへの書き込みは自宅PCで行い、書き込み済みのボードを学校に持ってくる**運用に切り替えます）。
