...
**所要時間の目安**：初回は1.5〜2時間（esp32ボードパッケージのダウンロードに時間がかかります）
**前提**：Windows 11（64bit）、インターネット接続、自分のPCの管理者権限（ドライバインストールで必要になることがあります）、GitHubアカウント
...
> **学校PCにも、同じ手順で導入します。** 自宅PC分はこの手順書どおり事前宿題として各自で、学校PC分はDay 0（8/17）の残り時間と8/18（火）の放課後に**チームで一緒に**導入します（私有USBメモリ・個人PCは学校ルールで持ち込み禁止のため、インストーラは各自が学校回線で公式サイトからダウンロードします。回線が混まないよう、開始のタイミングをずらします）。**ただし、GitHubアカウントまわり（0章）はアカウントに対して1回やれば済む**ので、自宅で終わっていれば学校PCでは不要です。**学校PCでは1章「Arduino IDE 2.x のインストール」から始めてください**（2つ目のアカウントを作ってしまわないよう注意してください）。また、学校のネットワークは**プロキシ**（外部との通信を中継するサーバーです。くわしくは1-2で説明します）を経由する構成のため、**学校PCでは 1-2 と5章の手順4のプロキシ設定が必須です**（やらないと、Arduino IDEが起動画面のまま止まり、cloneやpushも時間切れで失敗します）。学校PCは1人1アカウントなので、5章の初回pushで出るGitHubの認証は、自宅と同じように各自で行って大丈夫です。管理者権限の画面が出て進めなくなったときは、企画者に声をかけてください（8/17に企画者が学校PC1台で導入できるか下見をしました。1-2と5章の手順4のプロキシ設定は、この下見で判明した学校ネットワークの構成を反映した手順です。もしドライバが学校PCに入れられない場合は、**ボードへの書き込みは自宅PCで行い、書き込み済みのボードを学校に持ってくる**運用に切り替えます）。

手順は0章の確認と、そのあとの1〜8章です。集約担当だけに必要なステップには「（集約担当のみ）」と書いてあります。それ以外は全員共通です。

---

## 0. GitHubアカウントについて

アカウントの作り方そのものは公式の案内が最新なので、ここには置いていません。

このチームで決めていたのは、**コミット（変更内容の記録）に個人のメールアドレスを残さない**という1点です。
GitHubの Settings → Emails →「Keep my email addresses private」をOnにすると
`番号+ユーザー名@users.noreply.github.com` の形の代理アドレスが表示されるので、それを5章のgit設定で使います。
公開リポジトリでは、いちど記録されたメールアドレスは変更履歴に恒久的に残るためです。
...
1. まず、学校のプロキシの「IPアドレス」と「ポート番号」を控えます。`Windowsキー + I` で設定を開き、`ネットワークとインターネット` → `プロキシ` → 「手動プロキシ セットアップ」の「**編集**」をクリックすると、「プロキシ IP アドレス」（`proxy.example.invalid:8080` のような数字4組）と「ポート」（`8080` のような数字）が表示されるので、**2つともメモします**。メモしたら「キャンセル」で閉じてください（値を変えたり保存を押したりはしません。見てメモするだけです）。
...
4. 開いた中身を**全部消して**、次の4行に丸ごと置き換えます。
   ...
   > **⚠ このままコピーして貼らないでください。** 最後の行の `proxy.example.invalid:8080` と `8080` の部分は、**手順1でメモした自分の学校の値に書き換えます**。行頭の字下げ（半角スペース2個）はこの形のまま崩さないでください。
...
> プロキシ設定例の `proxy.example.invalid:8080` は説明用です。そのまま接続せず、学校の管理者から案内されたホスト名とポートへ置き換えてください。
...
4. **学校PCのみ**：Git にもプロキシを設定します（**自宅PCでは実行しないでください**）。1-2と同じ理由で、Git も学校のプロキシを知らないと外部と通信できません。ターミナルに以下を入力してEnterを押します。
   ...
   ```
   git config --global http.proxy http://proxy.example.invalid:8080
   ```

   > **⚠ このままコピーして貼らないでください。** `proxy.example.invalid:8080` と `8080` の部分は、1-2の手順1でメモした自分の学校の値に書き換えます。
...
3. 最後に、Gitがコミットメッセージなどの入力を求めるときに開くエディタを、VSCodeにしておきます。以下を実行します。

   ```
   git config --global core.editor "code --wait"
   ```

   （読み下し：Gitが「メッセージを書いてください」とエディタを開く場面で、VSCodeを開くようにする、この先ずっと使う設定です。これをしないと、Git標準の Vim（ヴィム）という操作方法が特殊なエディタが開き、閉じ方を知らないと画面が固まったように見えて詰まります。もしVimが開いてしまったときは、`Esc` を押してから `:wq` と打って `Enter` で抜けられます。）
...
2. `ファイル`（File）→ `開く`（Open...）で、clone した `classroom-temp-map` フォルダの中の `firmware/node-template/node-template.ino` を開きます。画面上部のタブに `node-template.ino` と `config.h` の2つが並んでいれば、正しく開けています。
...
| commit や pull の途中で黒い画面の見慣れないエディタが開き、操作できなくなった | Gitの既定エディタ（Vim）が開いて、コミットメッセージの入力を待っている | `Esc` を押してから `:wq` と打って `Enter` で、保存して抜けられます。5章「名前とメールを設定する」手順3の `git config --global core.editor "code --wait"` を実行しておくと、次からはVSCodeが開くようになります |
